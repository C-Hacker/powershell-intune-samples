...
write-host "Checking for $myString"
foreach($DCP in $DCPs) {
    
    write-host "Device Configuration Policy:"$DCP.displayName -f Yellow

    If ( $DCP."@odata.type" -like '*windows10CustomConfiguration*') {

        Write-Host "Custom Profile"
        If ( $DCP.omaSettings.omaUri -like  "*$myString*")  {
        
            write-host "Contains $myString" -f Green
            #write-host
        }#END If
        ...
        $DCP.PSObject.Properties | ForEach-Object {
            ...
            If ( $_.Name -like "*$myString*" ) {
                write-host $_.name  " = " $_.value -f Green
                #write-host
    ...
    if ( ( $DCP.displayName -ne "Baseline - ADMX - Chrome" ) -and ( $DCP.displayName -ne "Baseline - ADMX - Firefox" ) ) {
       #$DCP
